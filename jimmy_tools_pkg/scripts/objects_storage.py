...
class ObjectPositions:
    ...
    def callback(self, msg):
        for marker in msg.markers:
            ...
            if name in self.objects:
                ...
                if not isinstance(self.objects[name], list) or not all(isinstance(pos, (list, tuple)) for pos in self.objects[name]):
                    self.objects[name] = [self.objects[name]]  # Convert single entry to list

                existing_positions = self.objects[name]
                for existing_position in existing_positions:
                    if isinstance(existing_position, list):
                        existing_position = tuple(existing_position)  # Ensure tuple format

                    distance = math.sqrt(sum((p1 - p2) ** 2 for p1, p2 in zip(position, existing_position)))
                    if distance < 0.5:
    ...
    def save_periodically(self, event):
        self.save_to_file()
